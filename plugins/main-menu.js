...
const defaultMenu = {
  before: `👋🏻Halo Kak *%name*, Aku adalah Robot yang di Program menggunakan NodeJs untuk menjadi Asisten Virtual Auto-Reply di WhatsApp Jangan Lupa Baca Rules ya Kak di .rules
...
let anjay = await conn.sendFile(m.chat, './src/insecure.mp3', '', null, m, true, { type: "audioMessage", ptt: true, waveform: [1,4,8,9,18,23,17,14,12,15,7,3] }, { quoted: m })
  setTimeout(async () => { 7000
    await conn.sendMessage(m.chat, { delete: anjay.key })
    setTimeout(() => {
      conn.reply(m.chat, `Nggak jadi vn nya hehe,bercyanda bercyanda`,anjay)
    }, 1000)
  }, 5000)
}
handler.help = ['menu']
